class BinaryHeap:
    ...
    def _get_min_child(self, cur_idx):
        if 2 * cur_idx + 2 > len(self._heap) - 1:
            return 2 * cur_idx + 1
        if self._heap[2 * cur_idx + 1] < self._heap[2 * cur_idx + 2]:
            return 2 * cur_idx + 1
        return 2 * cur_idx + 2

    def _perc_down(self, cur_idx):
        while 2 * cur_idx + 1 < len(self._heap):
            min_idx = self._get_min_child(cur_idx)
            if self._heap[cur_idx] > self._heap[min_idx]:
                self._heap[cur_idx], self._heap[min_idx] = self._heap[min_idx], self._heap[cur_idx]
            else:
                return
            cur_idx = min_idx
